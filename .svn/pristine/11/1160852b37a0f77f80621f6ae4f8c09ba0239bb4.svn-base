 ...
 * @Last Modified time: 2016-06-01 19:46:29
 */
module.exports = {
    ...
    getImageData: function(img, callback) {
        ...
        if (img instanceof Image || img instanceof HTMLImageElement) {
            if (/^data\:/i.test(img.src)) { // Data URI
                var arrayBuffer = _this.base64ToArrayBuffer(img.src);
            ...
            var fileReader = new FileReader();
            fileReader.onload = function(e) {
                if (_this.debug) console.log("Got file of length " + e.target.result.byteLength);
                handleBinaryFile(e.target.result);
            };
